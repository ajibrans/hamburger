                ...
                <table>
                    ...
                    <%  excludedProps = Event.allEvents.toList() << 'version'
                        ...
                        <tr class="prop">
                            ...
                            <%  if (p.isEnum()) { 
								// custom
								if (org.springframework.context.MessageSourceResolvable.class.isAssignableFrom(p.type)) {%>
							<td valign="top" class="value"><g:message code="${p.type.name}.\${${propertyName}?.${p.name}}" default="\${${propertyName}?.${p.name}?.encodeAsHTML()}" /></td>
								<% } else { %>
							...
							<%  } else if(p.type == ([] as byte[]).class) { // custom %>
							<td><img src="\${createLink(controller: 'image', action: 'renderImage', id: ${propertyName}.id, params:['property':'${p.name}', 'domain':'${className}'])}"/></td>
							<%  } %>
						</tr>
                    <%  } %>
                    </tbody>
